...
const AppSidebar = () => {
  const { navigation, isActive, isSidebarOpen, setIsSidebarOpen, isHidden } =
    useSidebar();
  ...
  return (
    <aside
      ...
      } top-0 z-40 min-h-full transition-transform p-2 ${
        isSidebarOpen ? "w-72" : "w-24"
      } `}
    >
      {isHidden && (
        <div
          className={` ${isHidden ? "bg-amber-200" : "translate-x-0"} flex h-full flex-col bg-white/80 dark:bg-gray-800/80 backdrop-blur-2xl border-gray-200/50 dark:border-gray-700/50  rounded-3xl shadow-xl ${
